namespace _020_OOP2_020_Bankovni_ucty
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            double urok = 5.3;
            double pujcka = 5_000_000;
            double splatka = 25_000 * 12;

            SporiciUcet hypoteka = new SporiciUcet();
            hypoteka.UrokovaMira = urok;
            hypoteka.Uloz(pujcka);

            int roky = 0;
            while (hypoteka.Stav > splatka) 
            {
                Console.WriteLine($"rok {roky} : {hypoteka.Stav:0.00}");
                hypoteka.Urokuj();
                hypoteka.Vyber(splatka);
                roky++;
            }
            double zaplaceno = roky * splatka * hypoteka.Stav;
            Console.WriteLine($"roky = {roky}\nzaplaceno = {zaplaceno}");
        }
    }
}
